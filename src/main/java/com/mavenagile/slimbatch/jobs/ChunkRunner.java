...
public class ChunkRunner extends RunnerBase
{
	...
	protected boolean nextIteration() throws Exception
	{
		...
		if (aReadDO == null)
			return (false);
		// System.out.println(" I am running : nextIteration : " + getClass().getSimpleName() + " : " + aReadDO);

		oProcessed++;
		Object aProcessedDO = oProcessor.processItem(aReadDO);
		if (aProcessedDO != null)
		{
			// System.out.println("aProcessedDO do : " + aProcessedDO.toString());
	...
	{
//		 System.out.println(" I am running : lastIteration : " + getClass().getSimpleName() + " Processed : " + oProcessed + " Errored : " + oErrored);
		if (oCommitIndex > 0)
			aDBSession.commitTransaction();
		aDBSession.close();
//		System.out.println("	Step[" + oStep.getID() + "] All done with : " + oProcessed + " : " + oErrored + " commitIndex : " + oCommitIndex);
	}
