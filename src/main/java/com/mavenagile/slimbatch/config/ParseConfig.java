...
public class ParseConfig
{
	...
	private static String SCHEMA_NAME = "slim-batch.xsd";
	...
	private DocumentBuilder getDocumentBuilder()
	{
		if (oParser == null)
		{
			try
			{
				...
				Schema aSlimBatchSchema = aSchemaFactory.newSchema(ParseConfig.class.getResource(SCHEMA_NAME));
				...
				oParser.setEntityResolver(new EntityResolver()
					{
						...
						{
							if (varSystemId.endsWith(SCHEMA_NAME))
								return new InputSource(ParseConfig.class.getResourceAsStream(SCHEMA_NAME));
							else
								return null;
						}
					});
	...
	private Document getDocument(String varResourcePath) throws Exception
	{
		...
		{
			LOGGER.error("Configuration error : " + e.getMessage(), ExceptionHelper.getRootCause(e));
			throw e;
		}
	}
	...
	private Configuration getConfiguration(String varResourcePath) throws ConfigurationException, Exception
	{
		...
		catch (Exception e)
		{
			if (e instanceof ConfigurationException == false)
				throw new ConfigurationException(e, "Invalid Resource Path [" + varResourcePath + "] : " + ExceptionHelper.getRootCause(e).getMessage());
			else
				throw e;
		}
